...
// Uses VITE_API_URL from .env, appends /api/v1 for the gateway routes
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000';
const API_BASE_URL = `${API_URL}/api/v1`;

// Log base URL once in dev tools to help debug misconfig / network errors
if (typeof window !== 'undefined') {
  // eslint-disable-next-line no-console
  console.log('[Analyze] API_BASE_URL resolved to:', API_BASE_URL);
}
...
export const analyzeText = async (text) => {
  ...
  try {
    ...
    console.error('[Analyze] Text request failed:', error);
    throw error;
  }
};
...
export const analyzeURL = async (url) => {
  ...
  try {
    ...
    console.error('[Analyze] URL request failed:', error);
    throw error;
  }
};
